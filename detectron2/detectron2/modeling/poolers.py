...
def _convert_boxes_to_pooler_format(boxes: torch.Tensor, sizes: torch.Tensor) -> torch.Tensor:
    ...
    #reimplemented without repeat_interleave function (used numpy repeat instead)
    range_val = torch.arange(len(sizes), dtype=boxes.dtype, device=boxes.device)
    range_numpy = range_val.cpu().detach().numpy()
    sizes_numpy = sizes.cpu().detach().numpy()
    range_numpy_repeat = np.repeat(range_numpy, sizes_numpy)
    indices = torch.from_numpy(range_numpy_repeat)
    indices = indices.to(boxes.device)
    # indices = torch.repeat_interleave(torch.arange(len(sizes), dtype=boxes.dtype, device=boxes.device)) code before
